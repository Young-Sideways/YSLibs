...
static inline size_t _private_hashtable_table_memory_align(size_t size) {
    ...
    return (size_t)pow(2.0, ceil(log2((double)size))) - (size_t)1;     //!< return nearest (2^n - 1) > init_size (7, 15, 31, 63, 127 ...)
}
...
hashtable_t ht_init(size_t size, size_t key_size, size_t value_size, hasher_pt hashfunc) {
...
void set_hasher(hashtable_t table, const hasher_pt hasher) {
    ...
    struct hashtable_t* _table = table;
